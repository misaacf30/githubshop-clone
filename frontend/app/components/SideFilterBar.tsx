...
export const SideFilterBar = ({ categoryList, sizeList, colorList, page }: Props) => {
    ...
    return (
        <div className="min-[838px]:pr-[20px] ">
            <strong className='text-[14px] text-[#4a4a4a] pt-[10px] pb-[20px] pr-[10px] tracking-[0.5px] leading-[18px] block'>
                Shopping options
            </strong>
            <div className="block">
                {(categoryList && categoryList.length > 0) && (
                    <div className='block divide-y-[1.5px]'>
                        <h3 className='text-[14px] py-[8px] tracking-normal leading-[20px]'>
                            Category
                        </h3>
                        <ul className='text-[12px] text-[#494949] py-[10px]'>
                            {categoryList.map((category: any, index: number) => (
                                <li key={index} className='flex pl-[10px] pr-[7px] py-[4px] my-[8px] items-start'>  {/* flex!!! */}s
                                    <input
                                        type="checkbox"
                                        value={category.slug}
